...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    if (isset($_GET['delete']) && (int)$_GET['delete'] > 0){
        $query2 = "delete from temp_sales where temp_id = '".(int)$_GET['delete']."' ";
        $conn->query($query2);
 ...
 for( $j=0 ; $j<sizeof($item) ; $j++)
{
   ...
   if(empty($p))
   {
    ...
    $qu = "select * from products where name = '$i'" ;
    $rem = $conn->query($qu) ;
    $re = $rem->fetch_assoc() ;
 ...
 $qu = 0 - $q ;
  ...
  $conn->query($query) ;
   ...
   $k = "select id from products where name = '$i' " ;

     $reme = $conn->query($k) ;
    $resy = $reme->fetch_assoc() ;
   ...
   $query = "insert into sales(product_id,qty,price,date,bno,cust) values ('$id','$q' ,'$p' , now() ,  '$b_no','$cust')" ;

  
  $conn->query($query) ;

  update_product_qty($q,$id) ;
    $conn->query($query) ;
...
if(empty($y))
{
                    ...
                    $query = "select * from products where name = '$i'" ;
...
if($r == '1')
{
    header("location : add_ssales.php");
                echo "<script>window.location = \"add_ssales.php\"; </script>";
}
...
<div class="row">
 ...
 <h1  style="margin-left: 200px ; margin-bottom: 50px">ADD GODOWN SALE</h1>
...
update entries-->&nbsp x</b></h4>
 ...
 <form style="margin-bottom: 150px" method="post" action="add_ssales.php" >
  ...
  <div class="col-md-12">
    <div class="panel panel-default">
      ...
      <div class="panel-body">
        ...
        <form method="post" action="add_ssales.php">
...
<tbody  >
     ...
     if(isset($_POST['sibby']))
     {
      ...
      $q = "select * from products where name = '$tp' " ;
      $res = $conn->query($q) ;
      $r = $res->fetch_assoc() ;
     $price = $r['sale_price'] ;
     ...
     if(empty($price))
     {
      $q = "insert into temp_sales(name,pric,quantity) values('$tp','0','0')" ;
      $conn->query($q) ;
    }

    else
    {
            $q = "insert into temp_sales(name,pric,quantity) values('$tp','$price','0')" ;
      $conn->query($q) ;
    }
...
<td><?php echo $row['quantity'] * $row['pric']  ?></td>
  <td><a class="btn btn-primary" href="add_ssales.php?delete=<?php echo $row['temp_id']; ?>">delete</a></td>
